...
(function ($) {
  ...
  // Skip menu initialization if React is handling it (check for data attribute)
  if (!document.body.hasAttribute("data-react-menu")) {
  ...
  window.initPortfolioCarousel = function () {
    if ($(".tt-portfolio-carousel").length) {
      $(".tt-portfolio-carousel").each(function () {
        ...
        if ($ttPortfolioCarousel.is("[data-pagination-type]")) {
          var $dataPaginationType =
            $ttPortfolioCarousel.data("pagination-type");
        } else {
          var $dataPaginationType = "fraction"; // by default (bullets/fraction/progressbar)
        }
  ...
  $(".copy-link-btn").on("click", function (e) {
    ...
    // Show tick icon
    $icon.removeClass("fas fa-link").addClass("fas fa-check");

    // Revert back to link icon after 1 second
    setTimeout(() => {
      $icon.removeClass("fas fa-check").addClass("fas fa-link");
    }, 1000);
  ...
  // ================================================================
  // Page nav initialization function (for client-side navigation)
  // ================================================================

  window.initPageNavigation = function () {
    ...
    if (!isMobile) {
      ...
      $(".tt-page-nav")
        .find(".tt-pn-hover-title span")
        .each(function () {
          if (!$(this).parent().data("original-text")) {
            $(this).parent().data("original-text", $(this).text());
          }
        });

      $(".tt-page-nav").find(".tt-pn-hover-title span").remove();
      $(".tt-page-nav")
        .find(".tt-pn-hover-title")
        .each(function () {
          ...
          $(".tt-page-nav")
            ...
            .each(function () {
              ...
              if ($this.find("span").length === 0) {
                ...
                // Clone hover title
                var $pnHoverTitle = $this.find("span").first();
                for (var i = 0; i < 7; i++) {
                  $pnHoverTitle.clone().insertAfter($pnHoverTitle);
                }
              }
    ...
    // Reinitialize page nav hover for magic cursor
    console.log(
      "Checking for ball:",
      $("#ball").length,
      "isMobile:",
    ...
    // Check for magic cursor regardless of mobile detection, since it exists
    if ($("#ball").length && typeof gsap !== "undefined") {
      var $ball = $("#ball");
      var $ballWidth = 34;
      var $ballHeight = 34;
      var $ballOpacity = 0.5;

      console.log("Ball found, setting up hover handlers");

      // Clean up any old images from the ball first
      $ball.find(".tt-pn-image").each(function () {
        var $img = $(this);
        // Try to find the parent page nav to return it to
        var $parentNav = $(".tt-page-nav");
        if ($parentNav.length && $parentNav.find(".tt-pn-image").length === 0) {
          $img.appendTo($parentNav);
        } else {
          $img.remove();
        }
      });
      ...
      $(".tt-page-nav")
        .find(".tt-pn-link")
        .off("mouseenter mouseover mouseleave");

      // Page nav hover
      $(".tt-page-nav").each(function () {
        var $pageNav = $(this);
        console.log(
          "Processing page nav, has image:",
          $pageNav.find(".tt-pn-image").length,
        );
        if ($pageNav.find(".tt-pn-image").length) {
          // Clone the image for display, keep original in place
          var $originalImage = $pageNav.find(".tt-pn-image");
          console.log("Found image element:", $originalImage.get(0));

          $pageNav
            .find(".tt-pn-link")
            .on("mouseenter", function (e) {
              console.log("Mouse enter on page nav link");
              // Remove any old images from ball first
              $ball.find(".tt-pn-image").remove();

              $("#magic-cursor").addClass("tt-pn-hover-on");
              // Clone the image instead of moving it
              var $imageClone = $originalImage.clone();
              console.log("Cloning image to ball");
              $imageClone.appendTo($ball);
              console.log("Ball now contains:", $ball.html());
              gsap.to($ball, {
                duration: 0.3,
                width: "20vw",
                height: "20vw",
            ...
            .on("mouseleave", function (e) {
              console.log("Mouse leave on page nav link");
              $("#magic-cursor").removeClass("tt-pn-hover-on");
              $ball.find(".tt-pn-image").remove();
              gsap.to($ball, {
                duration: 0.3,
          ...
          console.log("Hover handlers attached to link");
        } else {
          $pageNav.find(".tt-pn-link").removeClass("not-hide-cursor");
          console.log("No image found in this page nav");
        }
      });
    } else {
      console.log("Ball or gsap not available");
    }
  };
})(jQuery);
